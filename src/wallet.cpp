...
bool CWallet::MultiSend()
{
...

    {
         ...
         BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if(!ExtractDestination(out.tx->vout[out.i].scriptPubKey, destMyAddress)) continue;

            if (nBestHeight <= nLastMultiSendHeight ) 
                    return false;   
            if (out.tx->IsCoinStake() && out.tx->GetBlocksToMaturity() == 0  && out.tx->GetDepthInMainChain() == nCoinbaseMaturity+2)
            {
                ...
                //get the fee amount
                //CWalletTx wtxdummy;
                //CreateTransaction(vecSend, wtxdummy, reservekey, nFeeRet, nSplitBlock, strFailReason, cControl);
                //nLastSendAmount = vecSend[vecSend.size() - 1].second;
                //if(nLastSendAmount < nFeeRet + 500)
                //{
                //    printf("%s: fee of %s is too large to insert into last output\n");
                //    return false;
                //}
